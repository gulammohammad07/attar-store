    ...
    <div className="overflow-hidden rounded-2xl border border-[#174a63]/10 bg-white shadow-sm">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {brands.map((brand) => (
              <tr key={brand.id} className="border-t border-[#174a63]/10 hover:bg-[#f8fcfe]">
                <td className="p-4 text-sm font-medium text-[#174a63]">{brand.name}</td>
                <td className="p-4 text-sm text-[#174a63]/60">{brand.slug}</td>
              </tr>
            ))}

            {brands.length === 0 && (
              <tr>
                <td colSpan={2} className="p-8 text-center text-[#174a63]/50">
                  No Brands
                </td>
              </tr>
            )}
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="font-medium text-[#174a63]">{brand.name}</h3>
                  <p className="mt-1 text-xs text-[#174a63]/50">{brand.slug}</p>
                </div>
              </div>
            </div>
          ))
