...
export default function ProductReviews({ product }: { product: Product }) {
  ...
  return (
    <div className="grid gap-12 lg:grid-cols-[320px_1fr]">
      {/* Summary */}
      <div>
        <div className="rounded-3xl border border-[#1c1712]/10 bg-white p-8 text-center">
          <p className="font-display text-6xl font-semibold text-[#1c1712]">
            {product.rating}
          </p>
          ...
          <div className="mt-6 space-y-2">
            {distribution.map((d) => (
              <div key={d.stars} className="flex items-center gap-3">
                <span className="w-8 text-right text-xs text-[#1c1712]/50">
                  {d.stars}★
                </span>
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-[#1c1712]/10">
                  <div
                    className="h-full rounded-full bg-gold"
                    style={{ width: `${d.percent}%` }}
                  />
                </div>
                <span className="w-8 text-xs text-[#1c1712]/40">
                  {d.percent}%
                </span>
              </div>
            ))}
        ...
        <button
          ...
          className="mt-4 w-full rounded-full bg-[#1c1712] py-3.5 text-sm font-medium text-white transition-colors hover:bg-gold"
        >
          Write a Review
        </button>
        ...
        {showForm && (
          <motion.div
            ...
            className="mt-4 rounded-2xl border border-[#1c1712]/10 bg-white p-6"
          >
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-[#1c1712]/15 p-3 text-sm focus:border-gold focus:outline-none"
            />
            <button
              type="button"
              onClick={() => {
      ...
      <div className="space-y-6">
        {reviews.map((review, index) => (
          <motion.div
            ...
            className="rounded-2xl border border-[#1c1712]/10 bg-white p-6"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1c1712] font-display text-lg text-gold">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#1c1712]/40">
            ...
            <p className="mt-4 text-sm leading-relaxed text-[#1c1712]/70">
