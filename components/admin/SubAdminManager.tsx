...
export default function SubAdminManager({ users, currentUserRole }: SubAdminManagerProps) {
  ...
  if (tableUsers.length === 0) {
    return (
      <div className="rounded-xl border border-[#111111]/10 bg-white p-8 text-center text-sm text-[#111111]/50">
        {emptyMessage}
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#111111]/10">
        ...
        <table className="w-full min-w-[640px]">
          <thead className="bg-[#f5f5f0]">
            <tr>
              ...
              <th className="p-4 text-left text-xs font-semibold uppercase tracking-wider text-[#111111]/60">Email</th>
              <th className="p-4 text-left text-xs font-semibold uppercase tracking-wider text-[#111111]/60">Role</th>
          ...
          <tbody>
            {tableUsers.map((user) => (
              <tr key={user.id} className="border-t border-[#111111]/10 hover:bg-[#f5f5f0]">
                <td className="p-4 text-sm font-medium text-[#111111]">{user.name}</td>
                <td className="p-4 text-sm text-[#111111]/70">{user.email}</td>
                <td className="p-4">
                  {isAdmin ? (
                    <select
                      value={user.role}
                      onChange={(e) => handleRoleChange(user.id, e.target.value)}
                      className="rounded-lg border border-[#111111]/15 bg-white px-3 py-2 text-sm text-[#111111] focus:border-black focus:outline-none"
                    >
                    ...
                    <span className="text-sm text-[#111111]/70">{user.role}</span>
                  )}
                </td>
      ...
      {/* Mobile cards */}
      <div className="divide-y divide-[#111111]/10 md:hidden">
        {tableUsers.map((user) => (
          <div key={user.id} className="p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-medium text-[#111111] truncate">{user.name}</p>
                <p className="text-xs text-[#111111]/60 truncate">{user.email}</p>
              </div>
              {isAdmin && (
                <button
                  type="button"
            ...
            <div>
              {isAdmin ? (
                <select
                  value={user.role}
                  onChange={(e) => handleRoleChange(user.id, e.target.value)}
                  className="w-full rounded-lg border border-[#111111]/15 bg-white px-3 py-2 text-sm text-[#111111] focus:border-black focus:outline-none"
                >
                  <option value="ADMIN">Admin</option>
                  <option value="SUBADMIN">Sub Admin</option>
                  <option value="USER">User</option>
                ...
                <span className="inline-block rounded-full border border-[#111111]/15 bg-[#f5f5f0] px-3 py-1 text-xs font-medium text-[#111111]/70">
                  {user.role}
                </span>
              )}
            </div>
  ...
  return (
    <div className="rounded-2xl border border-[#111111]/10 bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center gap-3">
        <Shield className="text-black" size={22} />
        <div>
          <h2 className="text-2xl font-semibold text-[#111111]">Team Access</h2>
      ...
      {isAdmin && (
        ...
        <div>
          ...
          <input
            name="name"
            required
            className="w-full rounded-lg border border-[#111111]/15 bg-white px-3 py-2.5 text-sm focus:border-black focus:outline-none"
        ...
        <div className="flex items-end">
          <button
            type="submit"
            disabled={pending}
            className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#111111] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#1f1f1f] disabled:opacity-50"
          >
            <UserPlus size={16} />
            Add Sub-Admin
          </button>
      ...
      <div className="space-y-8">
        <section>
          <div className="mb-3">
            <h3 className="text-lg font-semibold text-[#111111]">Admins &amp; Sub-admins</h3>
            <p className="text-sm text-[#111111]/60">Users who can access the admin panel.</p>
          </div>
        ...
        {isAdmin && (
          <section>
            <div className="mb-3">
              <h3 className="text-lg font-semibold text-[#111111]">Registered Users</h3>
              <p className="text-sm text-[#111111]/60">Change a user’s role here to grant admin access.</p>
            </div>
