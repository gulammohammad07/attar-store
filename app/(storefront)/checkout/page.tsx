...
export default function CheckoutPage() {
  ...
  if (placed) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center gap-6 bg-[#F8FCFE] px-6 text-center">
        ...
        <p className="max-w-md text-sm leading-relaxed text-[#174A63]/55">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Link
            href="/account/orders"
            className="rounded-full bg-[#174A63] px-8 py-4 text-sm font-semibold text-white hover:bg-gold"
          >
            View My Orders
          </Link>
          <Link
            href="/shop"
            className="rounded-full border border-[#174A63] px-8 py-4 text-sm font-semibold text-[#174A63] hover:bg-[#174A63] hover:text-white"
          >
            Continue Shopping
          </Link>
        </div>
  ...
  if (status === "loading") {
    return (
      <div className="flex min-h-[70vh] items-center justify-center bg-[#F8FCFE]">
  ...
  if (!user) {
    return (
      <div className="flex min-h-[70vh] items-center justify-center bg-[#F8FCFE] px-4 py-16">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_380px]">
          <form onSubmit={handlePlaceOrder} className="space-y-8">
            ...
            <section className="rounded-3xl border border-[#174A63]/10 bg-white p-8">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <input
                  ...
                  placeholder="Email"
                  className="rounded-xl border border-[#174A63]/15 px-4 py-3 text-sm focus:border-gold focus:outline-none sm:col-span-2"
                />
                <input
                  required
                  name="phone"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex w-full items-center justify-center gap-2 rounded-full bg-[#174A63] py-4 text-sm font-semibold text-white transition-colors hover:bg-gold disabled:cursor-not-allowed disabled:opacity-60"
            >
              {submitting ? (
                <>
            ...
            <ul className="mt-6 space-y-4">
              {items.map(({ product, quantity }) => (
                <li key={product.id} className="flex justify-between gap-4">
                  <span className="text-sm text-[#174A63]/70">
                    {product.name}{" "}
                    <span className="text-[#174A63]/40">× {quantity}</span>
                  </span>
