...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (order.paymentStatus !== "PAID") await prisma.order.update({ where: { id: order.id }, data: { paymentStatus: "PAID", status: "CONFIRMED", razorpayPaymentId: paymentId, razorpaySignature: signature } });
